...
foreach($run in $runs) {
    ...
    while(-not $process.WaitForExit(1000)) {
        if($watch.Elapsed.TotalSeconds -gt 65) {Stop-Process -Id $process.Id;throw 'Owned G34 fixture exceeded bounded deadline; evidence retained.'}
    }
    $process.WaitForExit()
    "Run=$run ExitCode=$($process.ExitCode) Seconds=$($watch.Elapsed.TotalSeconds)" | Add-Content -LiteralPath "$target/exit.txt"
    if($process.ExitCode -ne 0) {throw 'G34 native client returned nonzero exit code.'}
    if(Test-Path -LiteralPath "$target/g34x-failure.log") {throw (Get-Content -LiteralPath "$target/g34x-failure.log" -Raw)}
    $expected=if($run -eq 0){36}else{2}
    $log=Get-Content -LiteralPath "$target/g34x-run-$run.log" -Raw
    if($log -notmatch "PASS steps=$expected frames=[1-9][0-9]* screenshots=$expected restart=$run") {throw 'G34 native sequence incomplete.'}
    $audit=Get-Content -LiteralPath "$target/source-resource-audit.log" -Raw
    foreach($field in @('ShadowMaps','ShadowViews','ShadowPipelines','ShadowBuffers','AOTargets','AOViews','AOPipelines','AOBuffers','SceneLightingResources','LightBuffers','LightingPipelines','GraphicsSettingsObjects','MaterialRuntimeObjects','PBRBindings','PBRPipelines','SourceTextures','SourceBuffers','AssetDocuments','GR2ReaderResources','VegetationAssets','VegetationInstances','VegetationRenderAssets','VegetationGeometry','VegetationFailures','AllCPUDeformationCalls','GPUFallbacks','SkinPreparationFailures','CollisionResources')) {
...
$log=Get-Content -LiteralPath "$target/g34x-run-0.log" -Raw
...
Write-Output "PASS G34 native lighting/A-B/UI/style/restarts/map/lifetime proof: $target"
