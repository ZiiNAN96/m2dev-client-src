# ZiiNAN: Diligent water rendering integration; private test folders, original asset packages unchanged.
param([Parameter(Mandatory=$true)][ValidatePattern('^[a-zA-Z0-9_-]+$')][string]$Name,[switch]$Normal,[string]$ClientBinary)
...
$target=Join-Path "$source/build/milestone8" $Name
...
if(-not $Normal) {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/water_entry.py" -Destination "$target/test-root/root/prototype.py"
    Copy-Item -LiteralPath "$PSScriptRoot/water_smoke.py" -Destination "$target/test-root/root/water_smoke.py"
