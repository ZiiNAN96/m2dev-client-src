# ZiiNAN: Isolated 5B fixture or original-package login runtimes, never overwrite a previous run.
...
$target=Join-Path "$source/build/milestone5b" $Name
...
if(-not $Normal) {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/actor_variants_entry.py" -Destination "$target/test-root/root/prototype.py"
    Copy-Item -LiteralPath "$PSScriptRoot/actor_variants_smoke.py" -Destination "$target/test-root/root/actor_variants_smoke.py"
