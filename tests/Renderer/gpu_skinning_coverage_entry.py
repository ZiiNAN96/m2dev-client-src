"""ZiiNAN: GPU skinning actor coverage; original assets and native instance replacement."""
...
app.Create("Metin2 GPU skinning B4-X test", width, height, 1)
...
log = builtins.old_open("gpu-skinning-coverage-test.log", "w")
...
class World(ui.Window):
    ...
    def populate(self, mount):
        chr.Destroy()
        self.actors = []
        for index, (race, kind) in enumerate([(race, 6) for race in range(8)] + OTHERS):
    ...
    def OnUpdate(self):
        ...
        if phase != self.phase:
            ...
            log.flush()
        seconds = elapsed - phase * 35
        step = min(6, int(seconds / 5))
        if step != self.step:
            if step == 6:
                self.populate(mount)  # Real same-VID spawn/despawn; no unsafe direct DismountHorse helper.
            for vid, race, kind in self.actors:
                chr.SelectInstance(vid)
                if kind == 6:
                    shape = (0, 3, 6, 9, 0, 0, 0)[step]
                    chr.ChangeShape(armors[race % 4][shape])
                    chr.SetHair(1001 + (race % 4) * 1000 + (1 if step == 2 else 0))
                chr.SetLoopMotion(chr.MOTION_RUN if step in (1, 2) else chr.MOTION_WAIT)
                if step == 3:
                    chr.PushOnceMotion(chr.MOTION_COMBO_ATTACK_1 if kind == 6 else chr.MOTION_NORMAL_ATTACK, 0.1)
                if step == 4 and kind == 0:
                    chr.PushOnceMotion(chr.MOTION_DAMAGE, 0.1)
                if step == 5 and kind == 0:
        ...
        self.position = (x, y, z)
        self.camera = (2400.0 if step < 5 else 6500.0, 25.0, math.sin(elapsed * .12) * 70.0)
        ...
        for index, (vid, race, kind) in enumerate(self.actors):
            chr.SelectInstance(vid)
            px, py = x + ((index % 4) - 1.5) * 380, y + ((index // 4) - 1.5) * 380
            chr.SetPixelPosition(int(px), int(py), int(background.GetHeight(px, py)))
        chr.Update()
        self.label.SetText("B4-X | %s | mount %d | shape/hair %d | CPU default / GPU opt-in" % (name, mount, step))
