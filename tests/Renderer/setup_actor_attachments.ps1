# ZiiNAN: Diligent actor attachment rendering: isolated fixture or original-package login runtimes, never overwrite a previous run.
...
$target=Join-Path "$source/build/milestone5c" $Name
if(Test-Path -LiteralPath $target) { throw 'Choose a new test directory; existing evidence is preserved.' }
...
if(-not $Normal) {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/actor_attachments_entry.py" -Destination "$target/test-root/root/prototype.py"
    Copy-Item -LiteralPath "$PSScriptRoot/actor_attachments_smoke.py" -Destination "$target/test-root/root/actor_attachments_smoke.py"
    ...
    if($LASTEXITCODE) { throw 'Fixture packaging failed.' }
    $pack="$target/pack"
}
