# ZiiNAN: Diligent SpeedTree rendering; private test folders, original asset packages unchanged.
...
$target=Join-Path "$source/build/milestone6" $Name
...
if(-not $Normal) {
    ...
    if($LASTEXITCODE) { throw 'Tree fixture packaging failed.' }
    $pack="$target/pack"
}
