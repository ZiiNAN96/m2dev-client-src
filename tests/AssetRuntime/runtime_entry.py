...
app.Create("ZiiNAN Asset Runtime D1-X", width, height, 1)
...
class World(ui.Window):
    ...
    def OnUpdate(self):
        ...
        chr.Update()
        self.label.SetText("D1-X %s: player / NPC / mob / boss / mount; hair + armor + weapon" % name)
        ...
        if shot and self.shot != (phase, shot):
            ...
            success, path = grp.SaveScreenShotToPath("d1x-world-%d-%s-" % (phase, shot))
            log.write("screenshot phase=%d camera=%s success=%d file=%s\n" % (phase, shot, bool(success), path))
            log.flush()
            if not success:
                raise RuntimeError("Required D1-X %s screenshot failed" % shot)
