...
$target = Join-Path $source "build-d1x/runtime/$Name"
if (Test-Path -LiteralPath $target) { throw 'A fresh D1-X evidence directory is required.' }
...
"SourceBinary=$binary`nSHA256=$hash`nArguments=--renderer-diagnostics`nAutomated fixture; login/window visuals are separate" |
...
Write-Output "D1-X fresh Release PID=$($process.Id) Runtime=$target"
$watch = [Diagnostics.Stopwatch]::StartNew()
while (-not $process.WaitForExit(1000)) {
    $process.Refresh()
    if ($watch.Elapsed.TotalSeconds -gt 150) {
        Stop-Process -Id $process.Id
        throw "Owned D1-X fixture exceeded 150 seconds; evidence retained at $target"
    }
...
if ($process.ExitCode -ne 0) { throw 'D1-X fixture exited with a nonzero code.' }
...
foreach ($phase in 0..2) {
    ...
    foreach ($distance in @('far', 'near')) {
        $shot = [regex]::Match($fixture, "screenshot phase=$phase camera=$distance success=1 file=(?<file>d1x-world-$phase-$distance-[^\r\n]+)")
