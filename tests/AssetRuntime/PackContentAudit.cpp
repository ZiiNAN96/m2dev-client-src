...
{
    try {
        Require(argc==5||argc==6,"source pack source-directory report.tsv OR compare/extend old-pack new-pack report.tsv allowed-virtual-path");
        const bool source=std::string_view(argv[1])=="source",extend=std::string_view(argv[1])=="extend";
        Require(source?argc==5:(argc==6&&(extend||std::string_view(argv[1])=="compare")),"mode");
        CPack before,after;Require(before.Load(argv[2]),"original pack load");
        std::map<std::string,TPackFileEntry> other;
        if(!source){Require(after.Load(argv[3]),"new pack load");for(const auto& e:after.GetIndex())Require(other.emplace(e.file_name,e).second,"duplicate new entry");Require(extend?other.size()>=before.GetIndex().size():other.size()==before.GetIndex().size(),"entry count changed");}
        ...
        std::size_t added=0;
        if(extend)for(const auto&[name,entry]:other)if(!paths.contains(name)) {
            TPackFile bytes;Require(after.GetFile(entry,bytes),"added entry load");
            report<<name<<'\t'<<0<<'\t'<<bytes.size()<<'\t'<<0<<'\n';
            std::cout<<"ADD "<<name<<'\n';++added;
        }
        report.flush();Require(bool(report),"report write");std::cout<<"PASS entries="<<paths.size()<<" changed="<<changed<<" added="<<added<<" mode="<<argv[1]<<'\n';return 0;
