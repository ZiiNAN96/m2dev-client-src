...
# This audit reads only artifacts from the clean C2-X build and writes evidence beside that build.
$repositoryRoot = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '../..')).Path
$buildRoot = (Resolve-Path -LiteralPath (Join-Path $repositoryRoot 'build-c2x-clean')).Path
    ...
    foreach ($location in $locations) {
        ...
        foreach ($match in $matches) {
            ...
            if (-not $resolvedFile.StartsWith($buildPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
                throw "Refusing artifact outside build-c2x-clean: '$resolvedFile'."
            }
...
$manifestPath = Join-Path $evidenceDirectory 'c2x-binary-audit.csv'
$importsPath = Join-Path $evidenceDirectory 'c2x-binary-imports.csv'
$summaryPath = Join-Path $evidenceDirectory 'c2x-binary-audit-summary.txt'
...
if ($violations.Count -ne 0) {
    throw "C2-X binary audit failed:`n$($violations -join "`n")"
}
