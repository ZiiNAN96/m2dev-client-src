"""Bounded G2 production-client proof. Fixed camera, actor pose and wind in A/B pairs."""
...
app.Create("G2-X Modern Lighting / Classic + Modern", width, height, 1)
...
try:
    with builtins.old_open("g2x-expected.json") as stream:
...
log = builtins.old_open("g2x-run-%d.log" % restart, "w")
try:
    with builtins.old_open("g2x-vegetation.flag"):
        vegetation_proof = True
except FileNotFoundError:
    vegetation_proof = False
steps = 2 if restart or vegetation_proof else 19
...
class World(ui.Window):
    ...
    def OnUpdate(self):
        if time.monotonic() - self.started >= 1.2 and (self.step < 0 or self.step in self.shots):
            self.step += 1
            if self.step >= steps:
                app.Exit()
                return
            stage = self.step
            name = "b1" if vegetation_proof or (not restart and stage in (13, 14)) else "a1"
            if name != self.map:
                self.Load(name)
            modern = (restart == 1) if restart else stage in (1, 3, 5, 7, 9, 11, 12, 14, 15, 16, 18)
            self.options.graphicsDialog.style.SelectItem(int(modern))
            assert systemSetting.GetGraphicsSettings()["style"] == int(modern)
            if not restart and stage in (16, 17):
            ...
            if not restart and stage in (10, 11, 12):
                for vid in (58200, 58202, 58206):
                    chr.SelectInstance(vid)
                    chr.SetLoopMotion((chr.MOTION_COMBO_ATTACK_1 if vid == 58200 else chr.MOTION_NORMAL_ATTACK) if stage == 12 else chr.MOTION_WALK)
            focus = 2 if stage in (4, 5) else 6 if stage in (6, 7) else 0
            distance = 900 if stage in (2, 3, 4, 5, 6, 7) else 6000 if stage in (0, 1, 8, 9, 13, 14, 15) else 1900
            self.camera = (distance, 20 if distance < 2000 else 35, 0)
            x, y, z = self.position
            self.focus = (x + focus * 220, y, z + 70)
            if vegetation_proof:
                # Existing native B1 pagoda tree, located from AreaData/property data.
        ...
        frozen = bool(restart) or self.step not in (10, 11, 12)
...
class Capture(ui.Window):
    ...
    def OnRender(self):
        ...
        if 0 <= world.step < steps and world.step not in world.shots and time.monotonic() - world.started > .45:
            success, path = grp.SaveScreenShotToPath("g2x-%d-%02d-" % (restart, world.step))
            ...
            log.write("capture=" + json.dumps(dict(step=world.step, map=world.map, image=path, lighting=proof), sort_keys=True) + "\n")
            log.flush()
...
with builtins.old_open("g2x-expected.json", "w") as stream:
