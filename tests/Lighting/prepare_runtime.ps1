param(
    ...
    [switch]$VegetationProof
)
$ErrorActionPreference='Stop'
$source=(Resolve-Path -LiteralPath "$PSScriptRoot/../..").Path
$original=(Resolve-Path -LiteralPath "$source/../m2dev-client").Path
$build=(Resolve-Path -LiteralPath $BuildDirectory).Path
$target=Join-Path $source "build/g2x/runtime/$Name"
if(Test-Path -LiteralPath $target) {throw 'Fresh G2-X evidence directory required.'}
New-Item -ItemType Directory -Path "$target/test-root","$target/pack","$target/log","$target/mark","$target/upload" | Out-Null
if($VegetationProof){'1' | Set-Content -LiteralPath "$target/g2x-vegetation.flag"}
...
if(-not $Manual) {
    ...
    $wrapped="import builtins, traceback`ntry:`n$indented`nexcept Exception:`n    with builtins.old_open('g2x-failure.log', 'w') as failure:`n        failure.write(traceback.format_exc())`n    import app`n    app.Exit()`n"
...
if($LASTEXITCODE -ne 0) {throw 'Private G2-X root packaging failed.'}
