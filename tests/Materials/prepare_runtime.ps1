...
$target=Join-Path $source "build/g1x/runtime/$Name"
if(Test-Path -LiteralPath $target) {throw 'Fresh G1-X evidence directory required.'}
...
if(-not $Manual) {
    Copy-Item -LiteralPath "$PSScriptRoot/fixtures" -Destination "$target/test-root/root/g1x" -Recurse
    Copy-Item -LiteralPath "$PSScriptRoot/../AssetRuntime/fixtures/f5x" -Destination "$target/test-root/root/f5x" -Recurse
    Copy-Item -LiteralPath "$PSScriptRoot/fixtures/character_pbr.glb" -Destination "$target/test-root/root/f5x/character.glb" -Force
    ...
    $wrapped="import builtins, traceback`ntry:`n$indented`nexcept Exception:`n    with builtins.old_open('g1x-failure.log', 'w') as failure:`n        failure.write(traceback.format_exc())`n    import app`n    app.Exit()`n"
...
if($LASTEXITCODE -ne 0) {throw 'Private G1-X root packaging failed.'}
