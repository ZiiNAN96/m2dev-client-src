...
bool Relevant(const std::string& s){return s.ends_with(".spt")||s.ends_with(".prt")||s.ends_with("areadata.txt")||s.ends_with("objectdata.txt")||s.ends_with(".msm");}
int main(int argc,char**argv){
    try{
        if(argc!=4)throw std::runtime_error("usage: VegetationPackAudit client-directory pack-order.txt output.json");
        ...
        for(const auto& [path,e]:entries){
            ...
            if(!path.ends_with(".spt")){w.Key("text");w.String(reinterpret_cast<const char*>(bytes.data()),static_cast<unsigned>(bytes.size()));}
