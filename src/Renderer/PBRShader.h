...
struct POutput {float4 position:SV_POSITION;float3 eye:TEXCOORD0;float3 normal:TEXCOORD1;
 ...
 float3 world:TEXCOORD6;float3 viewDirection:TEXCOORD7;};
...
POutput PBRVS(float3 position:ATTRIB0,float3 normal:ATTRIB1,float2 oldUV:ATTRIB2,float4 tangent:ATTRIB5,float2 uv:ATTRIB6) {
 ...
 o.position=mul(eye,PProjection);o.world=mul(float4(position,1),PWorld).xyz;
 o.normal=LightingTransformNormal(normal,(float3x3)PWorld);
 float3 cameraWorld=-mul(PView[3].xyz,transpose((float3x3)PView));o.viewDirection=cameraWorld-o.world;
 o.tangent=float4(mul(float4(tangent.xyz,0),PWorld).xyz,tangent.w*(determinant((float3x3)PWorld)<0?-1:1));o.uv=uv;
...
float3 SurfaceNormal(POutput i,bool front) {
 ...
 [branch] if((PFlags.x&2)==0)return front?n:-n;
 ...
 float3 t=float3(1,0,0),b=float3(0,1,0);
 ...
 if(abs(i.tangent.w)>.5&&identity) {
   ...
   float3 dx=ddx(i.world),dy=ddy(i.world);float2 tx=ddx(uv),ty=ddy(uv);
   float det=tx.x*ty.y-tx.y*ty.x;
   if(abs(det)<1e-10)return front?n:-n;
...
float4 PBRPS(POutput i,bool front:SV_IsFrontFace
 ...
 float3 n=SurfaceNormal(i,front),v=SafeNormal(i.viewDirection,n),l=SceneSunDirection.xyz,h=SafeNormal(v+l,n);
