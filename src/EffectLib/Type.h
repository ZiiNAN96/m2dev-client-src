...
#define GRAVITY			Math::Vector3(0.0f, 0.0f, -9.8f)
...
typedef struct SEffectPosition : public CTimeEvent<Math::Vector3>
{
	// For Bezier Curve
	int m_iMovingType;
	Math::Vector3 m_vecControlPoint;
} TEffectPosition;
...
typedef CTimeEvent<Math::Color>					TTimeEventTypeColor;
typedef CTimeEvent<Math::Vector2>					TTimeEventTypeVector2;
typedef CTimeEvent<Math::Vector3>					TTimeEventTypeVector3;
...
inline Math::Vector3 BlendSingleValue(float time, const TEffectPosition& low, const TEffectPosition& high)
{
	const float timeDiff = high.m_fTime - low.m_fTime;
	const float perc = (time - low.m_fTime) / timeDiff;
