...
void Report(CGraphicThingInstance& thing,const char* status)
{
    ...
    if(!statesReported) {
        statesReported=true;
        for(auto type:{D3DRS_ALPHABLENDENABLE,D3DRS_ALPHATESTENABLE,D3DRS_ZENABLE,D3DRS_ZWRITEENABLE,D3DRS_ZFUNC,
        ...
        for(auto type:{D3DSAMP_ADDRESSU,D3DSAMP_ADDRESSV,D3DSAMP_MINFILTER,D3DSAMP_MAGFILTER,D3DSAMP_MIPFILTER}) {
            DWORD value=0; STATEMANAGER.GetSamplerState(0,type,&value); diagnostics << "sampler " << type << '=' << value << '\n';
        }
    }
...
{
public:
    static bool Capture(StaticObjectDraw& d)
    {
        ...
        const bool textureOnly=Stage(0,D3DTSS_COLOROP)==D3DTOP_SELECTARG1;
        if(STATEMANAGER.GetRenderState(D3DRS_ALPHABLENDENABLE) || STATEMANAGER.GetRenderState(D3DRS_ALPHATESTENABLE) ||
           !STATEMANAGER.GetRenderState(D3DRS_ZENABLE) || !STATEMANAGER.GetRenderState(D3DRS_ZWRITEENABLE) ||
           ...
           Stage(1,D3DTSS_COLOROP)!=D3DTOP_DISABLE || (!textureOnly && Stage(0,D3DTSS_COLOROP)!=D3DTOP_MODULATE) ||
           ...
           (Stage(0,D3DTSS_ALPHAARG2)!=D3DTA_CURRENT && Stage(0,D3DTSS_ALPHAARG2)!=D3DTA_DIFFUSE) ||
        ...
        if(textureOnly) {
            ...
            for(DWORD i=1;i<8;++i) { BOOL other=FALSE; if(SUCCEEDED(ms_lpd3dDevice->GetLightEnable(i,&other)) && other) return false; }
...
bool CaptureStaticMapObjectDraw(Renderer::StaticObjectDraw& draw)
{
    return StateReader::Capture(draw);
}

void SubmitStaticMapObject(CGraphicThingInstance& thing)
{
    using namespace Renderer;
    ...
    if(!CaptureStaticMapObjectDraw(common)) { Report(thing,"excluded: render state outside opaque diffuse contract"); return; }
    // Validate every current LOD/model/material first: never draw half an unsupported object.
    for(DWORD i=0;i<thing.GetLODControllerCount();++i) {
        ...
        for(DWORD j=0;j<palette.GetMaterialCount();++j) {
            auto& material=palette.GetMaterialRef(j);
            ...
            case D3DFMT_A8R8G8B8: case D3DFMT_X8R8G8B8: case D3DFMT_A8B8G8R8: break;
            default: Report(thing,"excluded: legacy texture format outside 4A BC/RGBA subset"); return;
            }
        }
    }
    auto& object=objects[&thing];
    for(DWORD i=0;i<thing.GetLODControllerCount();++i) {
        ...
        for(auto* node=model->GetMeshNodeList(CGrannyMesh::TYPE_RIGID,CGrannyMaterial::TYPE_DIFFUSE_PNT);node;node=node->pNextMeshNode) {
            ...
            for(auto* group=node->pMesh->GetTriGroupNodeList(CGrannyMaterial::TYPE_DIFFUSE_PNT);group;group=group->pNextTriGroupNode) {
                ...
                if(!texture) texture=LoadTerrainTextureFile(name.c_str(),*renderer);
    ...
    Report(thing,"submitted: static rigid diffuse");
}
