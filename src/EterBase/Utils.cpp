...
{
	static std::string s_utf8TempName;
	s_utf8TempName = Platform::Filesystem::CreateTemporaryFileName(c_pszPrefix ? c_pszPrefix : "etb");
...
{
	strFileName = Platform::Filesystem::CurrentDirectory();
...
{
	return filename && Platform::Filesystem::Exists(filename);
}
...
void MyCreateDirectory(const char* pathUtf8)
{
	if (!pathUtf8 || !*pathUtf8)
		return;
	(void)Platform::Filesystem::CreateDirectories(pathUtf8);
}
...
class CDirRemover : public CDir
{
	public:
		...
		bool OnFile(const char* c_szPathName, const char* c_szFileName)
		{
			...
			(void)Platform::Filesystem::RemoveFile(strFullPathName);
			return true;
		}
		...
		static void RemoveAllDirectory()
		{
			...
			{
				(void)Platform::Filesystem::RemoveEmptyDirectory(*itor);
			}
...
void RemoveAllDirectory(const char* c_szDirectoryName)
{
	...
	{
		(void)Platform::Filesystem::RemoveEmptyDirectory(c_szDirectoryName);
	}
}
...
{
	r_str = Platform::Filesystem::ExecutablePath();
}
